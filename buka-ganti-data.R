...
prefixes = "satisfaction-pushback-payload-400-rtt-500"
# prefixes = "fairness-payload-400-rtt-500"
# prefixes = "satisfaction-accept-payload-400"
topo     = "7018.r0"
evils    = "140"
...
if (file_test("-f", filename)) {
 ...
 levels(data.all$Scenario) <- sub("^satisfaction-pushback$", "satisfaction-pushback-400-rtt-500", levels(data.all$Scenario))
  # levels(data.all$Scenario) <- sub("^fairness$", "fairness-400-rtt-500", levels(data.all$Scenario))
  # levels(data.all$Scenario) <- sub("^satisfaction-accept$", "satisfaction-accept-400", levels(data.all$Scenario))
  name = paste (sep="-", prefixes, "topo", topo, "evil", evils, "producer", producer)
  filename = paste(sep="", "results/",folder,"/process/", name, "-data.txt")
